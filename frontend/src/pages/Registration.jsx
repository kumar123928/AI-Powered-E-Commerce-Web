...
const googleSignup = async () => {
    ...
    try {
       ...
       toast.success("Google signup succesfully")
       console.log(result.data)
        getCurrentUser()
            navigate("/")
    } catch (error) {
...
<button disabled={loading} className={`w-[100%] h-[50px] rounded-lg flex items-center justify-center mt-[20px] text-[17px] font-semibold cursor-pointer ${loading ?"bg-gray-500" : "bg-[#6060f5]" } `}> {loading ? "Creating account..." : "Create Account"} </button>

<p className='flex gap-[10px]'>If you have any account?<span className='text-[#5555f6cf] text-[17px] font-semibold cursor-pointer' onClick={()=> navigate("/login")}>Login</span></p>
</div>
            </form>
        </div>
        </div>
