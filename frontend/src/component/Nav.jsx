...
import ShopContext, { ShopDataContext } from '../context/ShopContext';
...
const handleLogout = async () => {
  try {
    ...
    setUserData(null);  // Clear user context after logout
...
{!showSearch && <IoSearch className='w-[35px] h-[35px] text-[#000000] cursor-pointer' onClick=
...
{showSearch && <RiSearchFill className='w-[35px] h-[35px] text-[#000000] cursor-pointer' onClick={()=>setShowSearch (prev=> !prev)} />}
 ...
 <FaRegUserCircle className='w-[35px] h-[35px] text-[#000000] cursor-pointer' onClick={()=>setShowProfile(prev=>!prev)}/>
)}

<BsCart3 className='w-[32px] h-[35px] text-[#000000] cursor-pointer hidden md:block' onClick={()=> navigate("/cart")}/>

<p className='absolute w-[18px] h-[18px] items-center justify-center bg-black px-[15px] py-[2px] text-white rounded-full text-[9px] top-[10px] right-[23px] hidden md:block'>{getCartCount()}</p>
...
<div className='w-[100vw] h-[90px] flex items-center justify-between px-[20px] fixed bottom-0 left-0 bg-[#191818] md:hidden text-[12px]'>
  <button className='text-[white] flex items-center justify-center flex-col gap-[2px]' onClick={()=> navigate("/")} ><IoMdHome className='w-[28px] h-[28px] text-[white] md:hidden cursor-pointer' />Home</button>

<button className='text-[white] flex items-center justify-center flex-col gap-[2px]' onClick={()=> navigate("/collection")}><MdOutlineCollectionsBookmark className='w-[28px] h-[28px] text-[white] md:hidden cursor-pointer'  />Collections</button>

<button className='text-[white] flex items-center justify-center flex-col gap-[2px]' onClick={()=> navigate("/contact")}><MdPermContactCalendar className='w-[28px] h-[28px] text-[white] md:hidden cursor-pointer'  />Contact</button>

<button className='text-[white] flex items-center justify-center flex-col gap-[2px]' onClick={()=> navigate("/cart")}><BsCart3 className='w-[28px] h-[28px] text-[white] md:hidden cursor-pointer' />Cart</button>
